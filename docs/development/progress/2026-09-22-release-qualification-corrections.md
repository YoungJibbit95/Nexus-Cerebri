...
| CPIR 0.1 primary example | FIXED | `examples/request.json` is minimal current 0.2; the old bytes are retained as `examples/legacy-cpir-0.1.json`. Both have Core/API roundtrip and Node coverage; Lab legacy parity uses the named legacy input. Unknown major/minor pairs fail closed. |
