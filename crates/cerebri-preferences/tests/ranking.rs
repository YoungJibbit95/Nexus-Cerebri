...
fn evidence(source: PreferenceSource, seconds: i64) -> PreferenceEvidence {
    PreferenceEvidence { source, preferred_start: at(seconds), evidence: vec![] }
}
...
fn source_precedence_time_ties_duplicates_and_evidence_permutations() {
    ...
    let sources = [ExplicitCurrentRequest, SessionContext, PersonalLearned, GlobalLearned, Default];
    ...
    for (rank, higher) in sources.iter().enumerate() {
        for lower in &sources[rank + 1..] {
            let mut entries = [evidence(*lower, 0), evidence(*higher, 3600), evidence(*higher, 7200), evidence(*higher, 3600)];
            ...
            for a in 0..4 { for b in 0..4 { for c in 0..4 { for d in 0..4 {
                let indices = [a,b,c,d];
                if (0..4).any(|i| indices[i+1..].contains(&indices[i])) { continue; }
                let profile = PreferenceProfile { preferences: indices.map(|i| entries[i].clone()).to_vec() };
                let features = profile.ranking_features(at(0), 1, None);
                assert_eq!(profile.preferred_start().unwrap().preferred_start, at(3600));
...
fn whole_second_distance_and_shift_preserve_missingness_and_quantization() {
    ...
    assert_eq!(serde_json::to_value(&no_signal).unwrap(), absent_preference());
    let profile = PreferenceProfile { preferences: vec![evidence(PreferenceSource::ExplicitCurrentRequest, 0)] };
    for (millis, seconds) in [(0,0),(800,0),(-800,0),(1000,1),(-1000,1),(12345,12),(-12345,12)] {
        ...
        assert_eq!(feature.preferred_start_source(), Some(PreferenceSource::ExplicitCurrentRequest));
        assert_eq!(feature.shift_seconds(), seconds);
        assert_eq!(feature.mutation_count(), 1);
        assert_eq!(profile.features(start).distance_from_preferred_start_seconds, seconds);
        assert_ne!(feature, no_signal);
    }
    // Current chrono domain extremes remain representable, without clamping or new errors.
    let extreme = PreferenceProfile { preferences: vec![PreferenceEvidence {
        source: PreferenceSource::Default, preferred_start: Instant::MIN_UTC, evidence: vec![],
    }] }.ranking_features(Instant::MAX_UTC, 1, Some(Instant::MIN_UTC));
    let magnitude = (Instant::MAX_UTC - Instant::MIN_UTC).num_seconds().unsigned_abs();
...
fn required_nullable_json_presence_truth_table_and_option_collapse_regression() {
    ...
    struct OrdinaryOption { preferred_start_distance_seconds: Option<u64> }
    assert!(serde_json::from_str::<OrdinaryOption>("{}").unwrap().preferred_start_distance_seconds.is_none());
    ...
    for distance in [None, Some(Value::Null), Some(json!(0))] {
        for source in [None, Some(Value::Null), Some(json!("ExplicitCurrentRequest"))] {
            ...
            if let Some(value) = &distance { map.insert("preferred_start_distance_seconds".into(), value.clone()); }
            if let Some(value) = &source { map.insert("preferred_start_source".into(), value.clone()); }
            let valid = matches!((&distance, &source), (Some(d), Some(s)) if d.is_null() == s.is_null());
            let decoded = decode(wire.clone());
            assert_eq!(decoded.is_ok(), valid, "{wire}");
            if let Ok(domain) = decoded {
                assert_eq!(domain.preferred_start_distance_seconds(), distance.as_ref().and_then(Value::as_u64));
...
fn wire_rejects_missing_unknown_duplicate_malformed_and_incompatible_fields() {
    for key in ["schema_version", "preferred_start_distance_seconds", "preferred_start_source", "mutation_count", "shift_seconds"] {
        let mut wire = absent_preference();
        wire.as_object_mut().unwrap().remove(key);
        assert!(decode(wire).unwrap_err().to_string().contains("missing field"), "{key}");
    }
    for version in [json!({"major":0,"minor":0}), json!({"major":0,"minor":2}), json!({"major":1,"minor":1}), json!({"major":0}), json!({"major":0,"minor":1,"extra":0}), Value::Null] {
        let mut wire = absent_preference(); wire["schema_version"] = version;
    ...
    for (key, value) in [
        ...
        let mut wire = absent_preference(); wire[key] = value;
    ...
    let overflow = wire.replace("\"shift_seconds\":0", "\"shift_seconds\":18446744073709551616");
...
fn all_sources_and_numeric_boundaries_roundtrip_deterministically() {
    use PreferenceSource::*;
    for source in [ExplicitCurrentRequest,SessionContext,PersonalLearned,GlobalLearned,Default] {
        for distance in [0,3600,u64::MAX] {
            ...
            assert_eq!(serde_json::to_string(&serde_json::from_str::<RankingFeatureSet>(&encoded).unwrap()).unwrap(), encoded);
        }
    }
}
