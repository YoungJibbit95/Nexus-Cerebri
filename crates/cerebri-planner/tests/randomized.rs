    ...
    for seed in 0..32_i64 {
        for fractional_ms in [0, 200, 800] {
            for mode in 0..4 {
                ...
                let original = if seed % 3 == 0 {
                    None
                } else {
                    Some([0, 800, 1000, 5200][seed as usize % 4])
                };
                if let Some(original) = original {
                    input.context.objects[0].revision = Some(Revision(1));
                    input.context.objects[0].time.value =
                        FieldState::known(span(original, original + 1000));
                ...
                if seed % 2 == 0 {
                    input.preferences.preferences = [preferred, preferred + 1000, preferred]
                        .map(|ms| PreferenceEvidence {
                            source,
                            preferred_start: instant(ms),
                            evidence: evidence(),
                        })
                        .to_vec();
                    if source != Default {
                        input.preferences.preferences.push(PreferenceEvidence {
                            source: Default,
                            preferred_start: instant(0),
                            evidence: vec![],
                        });
                    }
                }
                ...
                let key = |ms: i64| {
                    (
                        if seed % 2 == 0 {
                            ((ms - preferred) / 1000).unsigned_abs()
                        } else {
                            0
                        },
                        mutations,
                ...
                assert_eq!(
                    result
                        .candidates
                        .iter()
                        .map(|c| (c.start - epoch).num_milliseconds())
                        .collect::<Vec<_>>(),
                    expected
                );
                assert_eq!(result.search_space.evaluated, input.budget.max_candidates);
                assert_eq!(
                    result.search_space.exhausted,
                    input.budget.max_candidates == 8
                );
                assert_eq!(
                    result.assessment,
                    if input.budget.max_candidates == 8 {
                ...
                for candidate in &result.candidates {
                    let (distance, mutations, shift, _) =
                        key((candidate.start - epoch).num_milliseconds());
                    let feature = &candidate.ranking_features;
                    assert_eq!(
                        feature.preferred_start_distance_seconds(),
                        (seed % 2 == 0).then_some(distance)
                    );
                    assert_eq!(
                        feature.preferred_start_source(),
                        (seed % 2 == 0).then_some(source)
                    );
                    ...
                    assert_eq!(
                        ...
                        CandidateOrderingKey {
                            preference_distance_seconds: distance,
                    ...
                    assert_eq!(
                        (
                            ...
                            candidate.shifted_seconds
                        ),
                        (distance, mutations, shift)
                    );
