...
<section class="panel timeline-panel" aria-labelledby="timeline-title">
  ...
  <div class="timeline-scroll"><div class="timeline">
    ...
    {#each result?.compilation?.occurrences.slice(0, 8) ?? [] as item}<div class="timeline-row"><span class="row-label" title={item.id}>{item.series_id}<small>Occurrence {item.occurrence.sequence}</small></span><div class="track"><span class="time-block busy-block" style={intervalStyle(item.occurrence.visible_range, horizon)} title={`${item.id}: ${item.occurrence.range.start} to ${item.occurrence.range.end}; ${item.provenance}`}>{time(item.occurrence.range.start)} — {time(item.occurrence.range.end)}</span></div></div>{/each}
  ...
  <div class="chart-legend"><span><i class="legend-swatch busy-swatch"></i>Known busy / occurrences</span><span><i class="legend-swatch candidate-swatch"></i>Feasible proposal</span><span class="muted">First 8 candidates and occurrences · all data available in JSON</span></div>
</section>
