...
export const sections = {
  ...
  cpir: {
    ...
    summary: 'CPIR 0.1 carries scope, context, knowledge state, provenance, constraints, preferences, policy and planning capability as separate structures.',
    ...
    research: 'The schema is an internal 0.1 contract, not a public stable v1 promise. New semantic sources must preserve precedence and fail-closed behavior.',
    ...
    bullets: ['Schema 0.1 is validated explicitly.', 'MISSING is not the same as UNRESOLVED.', 'Capability does not silently expand planning scope.']
  },
  planning: {
    ...
    research: 'Recurrence-to-snapshot compilation, dependency/cycle handling, hard-constraint composition and richer score decomposition belong to the next planner milestone.',
    status: 'Implemented' as StatusKind,
    truth: 'REAL' as TruthKind,
    visual: 'planning',
  ...
  time: {
    ...
    research: 'Monthly/yearly recurrence, exceptions and automatic recurrence integration into CPIR planning remain outside the current implemented boundary.',
    status: 'Implemented' as StatusKind,
    truth: 'REAL' as TruthKind,
    visual: 'time',
  ...
  roadmap: {
    ...
    technical: 'The completed Temporal Core is software 0.2.0. The next concrete planner target is 0.3.0 and focuses on snapshot compilation, dependencies and constraint composition.',
    ...
    bullets: ['0.2.0 Temporal Core is implemented.', '0.3.0 planner work is next.', 'Providers, learning and neural layers remain later milestones.']
  },
  developers: {
